    ...
    public static class FrameEditor {
                ...
                if (Input.MousePositionX < patternEditorRightBound) {
                    ...
                    if (Input.GetClick(KeyModifier._Any)) {
                        if (mouseInBounds(mrow, mcolumn - channelScroll * 8) && Input.GetDoubleClick(KeyModifier.None) == false) {
                            ...
                            {
                                if (Input.MousePositionY > 180 + (FrameRenderer.numOfVisibleRows - 1) * 7 && currentRow < thisFrame.GetLastRow())
                                    Move(0, 1);
        ...
        static bool mouseInBounds(int mrow, int mcolumn) {
            ...
            if (channelScrollbar.barisPressed || channelScrollbar.barWasPressed > 0)
                return false;
            return mcolumn >= 0 && (mrow - cursorRow + FrameRenderer.numOfVisibleRows - FrameRenderer.centerRow) > 0 && mcolumn < 96 && (mrow - cursorRow + FrameRenderer.centerRow) < 55 && mrow >= 0 && mrow <= thisFrame.GetLastRow();
        }
        /// <summary>
        /// Returns the top-most coordinate of the row 'row'. Used for rendering purposes
        /// </summary>
        public static int getStartPosOfRow(int row) {
            return (row + FrameRenderer.centerRow - currentRow) * 7 + 184;
        }
        /// <summary>
        /// Returns the bottom-most coordinate of the row 'row'. Used for rendering purposes
        /// </summary>
        public static int getEndPosOfRow(int row) {
            return (row + FrameRenderer.centerRow - currentRow) * 7 + 184 + 7;
        }
        ...
        /// <param name="my"></param>
        /// <returns></returns>
        public static int getRowFromGlobalY(int my) { return (my - 184) / 7 + currentRow - FrameRenderer.centerRow; }
