...
namespace WaveTracker.Tracker {
    ...
    public class WTModule {
            ...
            foreach (Frame frame in song.frames) {
                ...
                for (int row = 0; row < 256; ++row) {
                    for (int channel = 0; channel < module.ChannelCount; ++channel) {
                        pat[row, channel, CellType.Note] = ConvertCell(frame.pattern[row][channel * 5 + 0]);
                        pat[row, channel, CellType.Instrument] = ConvertCell(frame.pattern[row][channel * 5 + 1]);
                        pat[row, channel, CellType.Volume] = ConvertCell(frame.pattern[row][channel * 5 + 2]);
                        int effect = ConvertCell(frame.pattern[row][channel * 5 + 3]);
                        if (effect != WTPattern.EVENT_EMPTY) {
                            effect = Helpers.GetEffectCharacter(effect);
                        }
                        pat[row, channel, CellType.Effect1] = effect;
                        pat[row, channel, CellType.Effect1Parameter] = ConvertCell(frame.pattern[row][channel * 5 + 4]);
                    }
                }
            }
        ...
        static int ConvertCell(short cellValue) {
