...
namespace WaveTracker.UI {
    public class Visualizer : Clickable {
        private Piano piano;
        private OscilloscopeGrid oscilloscopeGrid;
        private MiniTrackerLayout miniTrackerLayout;
        private static Color[] waveColors;
        public Visualizer() {
            x = 0;
            y = 0;
        ...
        public static void GenerateWaveColors() {
            ...
            foreach (Wave w in App.CurrentModule.WaveBank) {
                ...
                waveColors[i] = isEmpty ? Color.Transparent : GenerateColorFromWave(w);
                ++i;
            }
        }

        private static Color GetColorOfWaveFromTable(int index, float morph) {
            return morph > 0.001f ? waveColors[index].Lerp(waveColors[(index + 1) % 100], morph) : waveColors[index];
        }

        private static Color GenerateColorFromWave(Wave w) {
            double dcOffset = 0;
        ...
        public class Piano : Clickable {
            ...
            private int PianoHeight {
                get {
                    return 24 * PianoNoteWidth * 120 / 600;
                }
            }
            ...
            public class ChannelState : IComparable {
                ...
                public Color GetColor(bool withVolume) {
                    return withVolume && color.A != 0 ? Helpers.Alpha(color, (int)volume.Map(0, 1, 10, 255)) : color;
                }
        ...
        public class OscilloscopeGrid : Clickable {
            ...
            public void Update() {
                borderWidth = App.Settings.Visualizer.DrawInHighResolution ? 2 : 1;
                int numOscsX = 3;
            ...
            public class Oscilloscope : Clickable {
                ...
                private void DrawOscCol(int x, int y, float min, float max, Color c, int size) {
                    if (min < max) {
        ...
        public class MiniTrackerLayout : Clickable {
            private const int LINE_HEIGHT = 7;
            private const int HEADER_HEIGHT = 18;

            private int NumVisibleRows {
                get {
                    return (height - HEADER_HEIGHT) / LINE_HEIGHT;
                }
            }
            ...
            public void Draw() {
                ...
                for (int i = 0; i < App.CurrentModule.ChannelCount; ++i) {
                    ...
                    string str = channelWidth == 35 ? "Ch " + (i + 1).ToString("D2") : "Channel " + (i + 1).ToString("D2");
            ...
            private void DrawRow(int x, int y, int frame, int row, bool currRow, int channelWidth) {
                // draw pattern events
            ...
            private void DrawPatternEventCompact(int frame, int row, int channel, int x, int y, int effectColumns, int alpha) {
            ...
            private void DrawPatternEventExpanded(int frame, int row, int channel, int x, int y, int effectColumns, int alpha) {
            ...
            private void DrawBubbleRect(int x, int y, int w, int h, Color c) {
